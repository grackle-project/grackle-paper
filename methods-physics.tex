...
while $I_j$ is the appropriate radiative rate.  Note, that the
...
ideal gas, and $m_{\rm H}$ is the mass of a hydrogen atom.  Since
...
\frac{\rho_{Z}}{\mu_{Z}},
\end{equation}
where $\rho$ is the total gas density, $\rho_{Z}$ is the metal
...
\end{eqnarray}
\citep{1989ApJ...342..306H}, where $T$ is the gas temperature and
...
significantly heat the gas if the total H$_{2}$ binding energy is
returned to the gas.  For this reason, we also include the
appropriate chemical heating term, as described in Section
\ref{sec:chemheat}.  We note that the amount of H$_{2}$ binding
