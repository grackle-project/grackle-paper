...
\subsection{Self-shielding of the UV Background}
\label{section:UVB-self-shielding}
Self-shielding of gas from the UV background may be universally important, particularly at lower densities where collisional excitation is sub-dominant. However, accounting for this effect directly would require full radiative transfer, which is often computationally infeasible. In many cosmological simulations, the UV background is commonly taken to be everywhere optically thin, which may not always be an appropriate assumption. If desired, the user may include one of three analytic self-shielding prescriptions which operate independently on each computational element. Each method stems from the analytic fits to radiative transfer simulations from \citet{2013MNRAS.430.2427R}. \ion{H}{1} self-shielding occurs at densities
...
where $\bar{\sigma}_{\nu}$ is the gray (spectrum averaged) absorption cross-section and $f_g$ is the absorber baryon fraction, which we take as $f_g = 0.17$ for simplicity. Both the ionization and photoheating rates are then attenuated due to self-shielding, as 
...
All three available methods are various applications of equations \ref{eq:nssh} and \ref{eq:gamma_shield}. The first includes self-shielding for \ion{H}{1} by applying these equations, leaving \ion{He}{1} and \ion{He}{2} optically thin. The second applies these equations for both \ion{H}{1} and \ion{He}{1}, leaving \ion{He}{2} optically thin. Finally, the third applies these equations for \ion{H}{1} and \ion{He}{1}, while ignoring \ion{He}{2} photoionization and photoheating from the UV background entirely. This is a common simplifying assumption in radiative transfer simulations that is generally a very good approximation \citep{2006agna.book.....O, 2010MNRAS.408.1945M, 2012MNRAS.421.2232F, 2013MNRAS.430.2427R}. In other words, when accounting for self-shielding, leaving \ion{He}{2} optically thin to the UVB is likely much worse than ignoring it entirely. By default self-shielding is turned off, as care must be taken in using approximate self-shielding. This is particularly true when the ionization rate is dominated by collisional excitation \citep{2013MNRAS.430.2427R}, as is the case at high densities or for low UV background ionizing rates.

While equations \ref{eq:nssh} and \ref{eq:gamma_shield} are redshift independent fitting formulae, the gray-averaged cross section, $\bar{\sigma}_{\nu}$ depends on the spectrum of the UV background, which does depend on redshift. Included in the Grackle data files are the pre-computed $\bar{\sigma}_{\nu}$ for \ion{H}{1}, \ion{He}{1}, and \ion{He}{2} at each redshift for both the \citet{2009ApJ...703.1416F} and \citet{2012ApJ...746..125H} UV background models using the frequency dependent photoionization cross sections from \citet{1996ApJ...465..487V}.\footnote{Source code containing the analytic fits given in \citet{1996ApJ...465..487V} were obtained from \texttt{http://www.pa.uky.edu/~verner/photo.html}}

\subsection{Ionization and Heating from Radiative Transfer Simulations}
\label{section:radiative-transfer}

We allow for the optional inclusion of arrays of \ion{H}{1}, \ion{He}{1}, and \ion{He}{2} ionization rates, and a $\rm{H}_2$ disassociation rate from radiative transfer calculations, independent of previously discussed ionization rates. Associated heating from these processes is handled through a single heating rate array. These rates are included for each computational element and tied to the overall heating/cooling and ionizing rates. This allows the user to couple radiative transfer solutions self-consistently with the chemical reaction network.
