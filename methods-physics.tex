...
Self-shielding of gas from the UV background may be universally important, particularly at lower densities where collisional excitation is sub-dominant. However, accounting for this effect directly would require full radiative transfer, which is often computationally infeasible. In many cosmological simulations, the UV background is commonly taken to be everywhere optically thin, which may not always be an appropriate assumption. If desired, the user may include one of three analytic self-shielding prescriptions which operate independently on each computational element. Each method stems from the analytic fits to radiative transfer simulations from \citet{2013MNRAS.430.2427R}. H self-shielding occurs at densities
...
We allow for the optional inclusion of arrays of H, He, and He$^+$ ionization rates, as well as a $\rm{H}_2$ disassociation rate, from radiative transfer calculations, independent of previously discussed ionization rates. Associated heating from these processes is handled through a single heating rate array. These rates are included for each computational element and tied to the overall heating/cooling and ionizing rates. This allows the user to couple radiative transfer solutions self-consistently with the chemical reaction network.
